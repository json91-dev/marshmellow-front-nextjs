...
import styles from './identifyCheck.module.scss';
...
export default function IdentifyCheck() {
  ...
  return (
    <div className={cx(styles.container, !isRunningPASS && styles.bgGray)}>
      {!isRunningPASS && (
        <>
          <Image src="/images/mallow.work.svg" alt="No Image" width={132} height={124} />
          <div className={styles.description}>입사지원을 위해 최초 1회 본인인증이 필요합니다.</div>
          <ConfirmButton
      ...
      {isRunningPASS && (
        <>
          <div className={styles.title}>현재 인증이 진행중입니다.</div>
          <div className={styles.description}>통신사 앱에서 인증 후, 아래 [확인]버튼을 꼭 누르세요.</div>
