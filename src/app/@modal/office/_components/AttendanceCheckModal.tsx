  ...
  return (
    <>
      ...
      <CSSTransition in={isShowAttendanceCheckModal} timeout={200} unmountOnExit classNames="modal" nodeRef={modalRef}>
        <div
          className={cx(styles.attendanceCheckModal, 'modal', isAttendanceCheckInOneMinute && styles.bgPink)}
          ref={modalRef}
        >
          <p className={styles.title}>업무를 시작해요</p>

          {isAttendanceCheckInOneMinute ? (
            <>
              <p className={styles.bonusTitle}>{'1분 이내로 업무를 시작해서\n뽀너스 마시멜로우를 획득할 수 있어요!'}</p>
              <Image
                className={styles.mallowImage}
            ...
            <Image className={styles.mallowImage} src="/images/mallow.happy.svg" alt="No Image" width={72} height={72} />
          )}

          <p className={styles.description}>{'광고를 끝까지 시청해야 업무 보상으로\n마시멜로우를 받을 수 있어요!'}</p>
          <button className={styles.confirmButton} onClick={onClickAttendButton}>
            <Image
